...
Microservice d'authentification JWT - partie de l'architecture microservices e-commerce, déployée sur **AWS (EKS Auto Mode + Helm)** ou en **self-hosted** (Kubernetes / Docker Swarm).

> 💡 **Objectif Portfolio** : Ce service illustre le pipeline CI/CD complet avec GitHub Actions - tests → build Docker → scan Trivy → push GitHub Container Registry (GHCR) → déploiement Helm.
